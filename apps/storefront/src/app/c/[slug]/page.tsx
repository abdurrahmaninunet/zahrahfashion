...
interface Filter {
  ...
  name: string;
  inputType: string;
  values: { label: string; value: string; hexCode: string | null; count: number }[];
}
interface Listing {
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    return {
      title: name,
      description: `Shop ${name} — pay with card, transfer or on delivery in Lagos.`,
    };
  } catch {
...
export default async function CategoryPage({
  ...
  for (const [key, value] of Object.entries(query)) {
    if (key.startsWith('f_') || ['sort', 'page', 'priceMin', 'priceMax'].includes(key)) {
  ...
  return (
    <div className="mx-auto max-w-[1905px] px-4 lg:px-[8rem] py-6">
      {listing.products.length ? (
        ...
        <p className="py-16 text-center text-sm text-stone-400">No products in this category yet.</p>
      )}

      {listing.total > listing.pageSize && (
        <div className="mt-8 flex justify-center gap-2 text-sm">
          {listing.page > 1 && <Link href={makeHref({ page: String(listing.page - 1) })} className="rounded-md border border-stone-200 bg-white px-4 py-2">← Previous</Link>}
          {listing.page * listing.pageSize < listing.total && <Link href={makeHref({ page: String(listing.page + 1) })} className="rounded-md border border-stone-200 bg-white px-4 py-2">Next →</Link>}
        </div>
      )}
    </div>
  );
